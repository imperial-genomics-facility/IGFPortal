...
from app.models import CosmxSlideQCData
from app.cosmx_qc_data_api import (
...
def test_load_cosmx_qc_report(db, tmp_path):
    temp_report_dir = \
    ...
    load_cosmx_qc_report(
        cosmx_slide_igf_id='test1',
        qc_tag='test 1',
        file_path=temp_report_path,
        base_path=temp_base_dir)
    # check if its loaded
    record = db.session.query(CosmxSlideQCData).filter_by(cosmx_slide_igf_id='test1').first()
    assert record is not None
    assert record.cosmx_slide_igf_id == 'test1'
...
def test_async_load_cosmx_qc_report(db, tmp_path):
    temp_report_dir = \
        tempfile.mkdtemp(dir=tmp_path)
    temp_base_dir = \
        tempfile.mkdtemp(dir=tmp_path)
...
def test_CosmxSlideQCDataApi1(db, test_client, tmp_path):
    res = \
        test_client.post(
    ...
    res = \
        test_client.post(
            '/api/v1/cosmx_qc_data/add_report',
            data=dict(file=(report_file_data, 'report.html'),cosmx_slide_igf_id="test1",qc_tag="test 1"),
            headers={"Authorization": f"Bearer {token}"},
            content_type='multipart/form-data')
    assert res.status_code == 200
    assert json.loads(res.data.decode('utf-8')).get("message") == 'successfully submitted cosmx qc report loading job for report.html'
