...
def test_run_metadata_json_validation():
    errors = _run_metadata_json_validation(
            ...
            with patch('app.raw_metadata_view.get_airflow_dag_id') as  mock_get_airflow_dag_id:
                with patch('app.raw_metadata_view.trigger_airflow_pipeline') as mock_trigger_airflow_pipeline:
                    with patch.dict('os.environ', {'AIRFLOW_CONF_FILE': 'test_conf'}):
                        ...
                        results = async_validate_metadata(
...
def test_compare_metadata_sample_with_db(db):
    ...
    try:
        ...
        db.session.rollback()
        raise
    metadata_errors = compare_metadata_sample_with_db(
        metadata_file="data/metadata_file1.csv"
    )
    err = "Sample sample105799 is linked to project test1, "
    err += "not IGFQ000001_cs_23-5-2018_SC"
    assert err in metadata_errors


def test_search_metadata_table_and_get_new_projects(db):
    metadata1 = RawMetadataModel(
        metadata_tag='test1',
        raw_csv_data='raw',
        formatted_csv_data='formatted',
        report=''
    )
    metadata2 = RawMetadataModel(
        metadata_tag='test2',
        raw_csv_data='raw',
        formatted_csv_data='formatted',
        report=''
    ...
    try:
        ...
        db.session.rollback()
        raise
    new_projects = search_metadata_table_and_get_new_projects(
