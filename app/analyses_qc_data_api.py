...
"""
    Analyses QC data Api
"""
log = logging.getLogger(__name__)

def load_analyses_qc_report(
        analysis_name: str,
        analysis_type: str,
        qc_tag: str,
        file_path: str,
    ...
    try:
        ...
        hash_string = \
            f"{analysis_name}{analysis_type}{qc_tag}{file_name}{datetime_str}"
        hash_md5 = \
            hashlib.\
        ...
        try:
            ...
            db.session.add(analyses_qc_entry)
            db.session.flush()
            db.session.commit()
        except:
            db.session.rollback()
            ...
            raise ValueError(
                f"Failed to load analyses qc report to db, error: {e}")


@celery.task(bind=True)
def async_load_analyses_qc_report(
        self,
        analysis_name: str,
        analysis_type: str,
        qc_tag: str,
...
class AnalysesQCDataApi(ModelRestApi):
    resource_name = "analyses_qc_data"
    ...
    def add_report(self):
        try:
            ...
            analysis_name = json_data.get('analysis_name')
            analysis_type = json_data.get('analysis_type')
            qc_tag = json_data.get('qc_tag')
            if analysis_name is None or \
               analysis_type is None or \
               qc_tag is None:
                return self.response_400('Missing analysis_name, analysis_type or qc_tag')
            ...
            _ = \
                async_load_analyses_qc_report.\
                    apply_async(args=[
                        ...
                        report_file,
                        base_dir])
            return self.response(200, message=f'successfully submitted analyses qc report loading job for {os.path.basename(report_file)}')
        except Exception as e:
            log.error(e)
