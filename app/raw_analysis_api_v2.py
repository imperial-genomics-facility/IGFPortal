...
from .models import RawAnalysisV2

log = logging.getLogger(__name__)

class RawAnalysisV2Api(ModelRestApi):
    resource_name = "raw_analysis_v2"
    ...
    def search_new_analysis(self):
        try:
            new_analysis_list = \
                db.session.\
                    query(RawAnalysisV2.raw_analysis_id).\
                    filter(RawAnalysisV2.status=='VALIDATED').\
                    all()
            new_analysis_ids = [
    ...
    def get_raw_analysis_data(self, raw_analysis_id):
        try:
            result = \
                db.session.\
                    query(
                        RawAnalysisV2.project_id,
                        RawAnalysisV2.pipeline_id,
                        RawAnalysisV2.analysis_name,
                        RawAnalysisV2.analysis_yaml).\
